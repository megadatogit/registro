...
/**
 * Preset por defecto (idéntico al del script original, con nombres iguales)
 */
const DEFAULT_CFG = {
  // ---- Style ----
  	bgDark: "#c0c0c0",   // Plateado
        bgLight: "#a6a6a6",  // Gris más fuerte para contraste
        line:   "#dcdcdc",    // Gris perla para triángulos,
  ...
  trail: 0.45, // fracción de la arista detrás de la cabeza
  seed: 20250819,
...
/**
 * Motor original adaptado a React: acepta canvas y cfg, y un modo de medición ("window" | "parent").
 * Devuelve un API con destroy() y setConfig().
 */
function createTriangularMesh(canvas, userCfg = {}, fit = "window") {
  ...
  function buildGraph() {
    ...
    const base = Math.max(
      cfg.baseMin,
      Math.min(cfg.baseMax, Math.floor(Math.min(w, h) / 7))
    );
  ...
  function maintainPopulation(now) {
    while (
      particles.length < cfg.targetParticles &&
      particles.length < cfg.maxParticles
    ) {
      spawnParticle(now);
    }
  }
  ...
  function drawDirGlow(dt) {
    ...
    for (const [k, obj] of [...dirGlow.entries()]) {
      ...
      grad.addColorStop(
        0.85,
        `rgba(${glowRGB[0]},${glowRGB[1]},${glowRGB[2]},${alpha * 0.85})`
      );
      grad.addColorStop(
        1.0,
        `rgba(${glowRGB[0]},${glowRGB[1]},${glowRGB[2]},${alpha})`
      );
  ...
  function nearMargin(x, y) {
    return (
      x <= cfg.killMarginPx ||
      x >= G.w - cfg.killMarginPx ||
      y <= cfg.killMarginPx ||
      y >= G.h - cfg.killMarginPx
    );
  }
  ...
  function drawParticles(dt) {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (
        x < -cfg.viewMargin ||
        x > G.w + cfg.viewMargin ||
        y < -cfg.viewMargin ||
        y > G.h + cfg.viewMargin
      ) {
        kill.push(i);
        continue;
      }
  ...
  // Si se ajusta al contenedor, observa cambios de tamaño del padre
  let ro = null;
  if (fit === "parent" && canvas.parentElement && "ResizeObserver" in window) {
...
/**
 * @param {{ fit?: "window"|"parent", config?: Partial<typeof DEFAULT_CFG> } & React.CanvasHTMLAttributes<HTMLCanvasElement> } props
 */
const TriangularMesh = forwardRef(function TriangularMesh(
  { fit = "window", config, className, style, ...canvasProps },
  ref
) {
  const canvasRef = useRef(null);
  const engineRef = useRef(null);

  // Memoizamos un objeto de config para evitar renders infinitos si nos pasan {} nuevo cada vez
  const mergedCfg = useMemo(
    () => (config ? { ...config } : undefined),
    // si quieres reconfigurar en caliente, cambia esta política:
  ...
  useEffect(() => {
    ...
    // Inicia motor
    engineRef.current = createTriangularMesh(canvasRef.current, mergedCfg, fit);
    ...
    // fit y mergedCfg dan lugar a reinicio limpio
  }, [fit, mergedCfg]);
